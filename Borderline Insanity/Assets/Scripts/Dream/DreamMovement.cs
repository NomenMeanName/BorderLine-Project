...
public class DreamMovement : MonoBehaviour
{
    ...
    {
        if (Input.GetKey("d") && blocking == false) // Right
        {
            rb.velocity = new Vector2(speed, rb.velocity.y); //Moves the player right
            transform.localScale = new Vector3(2, transform.localScale.y); // Makes the player look right
        }
        else if (Input.GetKey("a") && blocking == false)
        {
            rb.velocity = new Vector2(-speed, rb.velocity.y); //Moves the player left
            transform.localScale = new Vector3(-2, transform.localScale.y); // Makes the player look left
        }        
        else
        {
